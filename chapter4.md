...
你可以在每個列舉常數的逗號後選擇性地換行。在一個沒有方法及說明文件的而只有常數存在的列舉類別中，他可能被格式化成像是一個陣列的Initializer，如private enum Suit { Clubs, Hearts , Spades, Diamonds }。因為列舉也屬於一類別，因此也適用其他用來各式化類別的規則
