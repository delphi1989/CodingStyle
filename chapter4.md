...
像是其他的程式區塊一樣，switch區塊的本體也是以2個white space 作為縮排層級。
在每個switch標籤後都將換行，而下一行也將有2個white space 的縮排，就是像是一個小區塊的起始標記。而之後的每個switch標籤會恢復到原本的縮排層級（跟第一個標籤相同），像是一個小區塊的結束標記。
