...
此條件也適用於一些類似運算符號的其他符號，如點```.```、在類別限定Parameterized中的Ampersand(```&```)符號’<T extends ClassA & ClassB>’、在catch括號內的pipe符號’catch（SQLException | IOException）'

2.當遇到assignment的運算符號時，應在其後換行，但在其前換行亦還可以接受。如’=‘號，應在等號後方換行。
此條件也適用於一些類似assignment符號的其他符號，如’foreach'

3.建構子及方法的名稱應與起始小括號在同一行，其後再換行

4.逗號會與他前面的token在同一行，其後再換行
